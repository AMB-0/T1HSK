...
class binary_node()	:
	...
	def create_rChild(self, nodes, leftChildAbleNode, rightChildAbleNode, index):
		self.rightChild = binary_node(self)
...
class random_binary_tree():
	...
	# Transverse the binary tree without using recursion
	def transverse(self, Node):
		while Node is not None:
			parent = Node.get_parent()
			
			if Node.get_lVisited() is None or Node.get_lVisited() != Node.get_parent():
				if Node.get_lChild() is not None:
					next = Node.get_lChild()
					next.set_lVisited(Node)
				elif Node.get_rChild() is not None:
					next = Node.get_rChild()
					next.set_lVisited(Node)
				elif Node.get_rChild()
